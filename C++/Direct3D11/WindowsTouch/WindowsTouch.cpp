...
public:
    ...
    float GetBoundingSphereSquared() const
    {
        return m_fBoundingSphereSquared;
    };
...
int CaptureFullScreenFileD11(ID3D11Device *pDevice,ID3D11DeviceContext* pContext,IDXGISwapChain* pSwapChain,const char* filetosave)
{
...
void CALLBACK OnKeyboard(UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext)
{
    static int st_SuccCap=0;
    if(st_SuccCap == 0)
    {
        int ret;
        DEBUG_INFO("keyboard on\n");
        ID3D11Device *pDevice=NULL;
        IDXGISwapChain *pSwapChain=NULL;
        ID3D11DeviceContext *pContext=NULL;
        pDevice = DXUTGetD3D11Device();
        pSwapChain = DXUTGetDXGISwapChain();
        pContext = DXUTGetD3D11DeviceContext();

        DEBUG_INFO("device 0x%p swapchain 0x%p context 0x%p\n",pDevice,pSwapChain,pContext);
        if(pDevice && pSwapChain && pContext)
        {
            ret = CaptureFullScreenFileD11(pDevice,pContext,pSwapChain,"z:\\test2.bmp");
            if(ret >= 0)
            {
                st_SuccCap = 1;
            }
